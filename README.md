...
- **按模块个数退火（Annealing by Max Modules）**：
  - **高温期（large 步长，如 bootstrap / reignite）**：最多允许选择 **3 个机制模块**，支持跨维度协同跃迁与机制探索；
  - **中温期（medium 步长，如 climb / climb_wide）**：最多允许选择 **2 个强耦合机制模块**，进行同类结构或局部联动探索；
  - **低温期（small 步长，如 polish）**：严格退火为 **1 个单一机制模块**，进行单变量精细调优与消融；
...
- 主动弃权 / 靶点非法或冻结 / 模块数超出退火上限 / 层级越权 / 本轮候选靶点组合重复
- 缺 `phenomenon` 或 `evidence.sample_ids`（无现象支撑）、缺 `falsified_if`（不可证伪）
- 违反步长契约：`step_size` 不符、变量数超上限、`best_only` 却要从基座重开、
  large 缺竞争机制/区分方式
